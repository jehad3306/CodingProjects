...
def pollardRho(n):
    ...
    while True:
        ...
        # if gcd == 1, it means that we need to iterate to the next value of the functions til we find a non-trivial factor
        # if we iterated more than 10.000 times, we test out a new path.
        if(d == 1):
            nextValue()
...
print(f"The non-trivial factor:  {pollardRho(prime_test_n)}")
